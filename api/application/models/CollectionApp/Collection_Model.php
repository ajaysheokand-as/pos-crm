...
//            echo $this->db->last_query(); exit;
    ...
    public function get_basic_loan_details($lead_id = null) {
        $result_array = array();
        $conditions['LD.lead_id'] = $lead_id;
        ...
        if (!empty($tempDetails->num_rows())) {
            ...
            $result_array['status'] = 1;
            $data['lead_id'] = $this->encrypt->encode($columns['lead_id']);
            $data['loan_no'] = ($columns['loan_no']) ? $columns['loan_no'] : "-";
            $data['full_name'] = ($columns['full_name']) ? $columns['full_name'] : "-";
            $data['gender'] = ($columns['gender']) ? $columns['gender'] : "-";
            $data['dob'] = ($columns['dob']) ? date("d-m-Y", strtotime($columns['dob'])) : "-";

            $data['pancard'] = ($columns['pancard']) ? $columns['pancard'] : "-";
            $data['lead_applied_state'] = ($columns['customer_lead_applied_state']) ? $columns['customer_lead_applied_state'] : "-";
            $data['lead_applied_city'] = ($columns['customer_lead_applied_city']) ? $columns['customer_lead_applied_city'] : "-";
            $data['pincode'] = ($columns['pincode']) ? $columns['pincode'] : "-";
...
//            $conditions['LCV.col_visit_field_status_id'] = 5; // Final Visit Completed
    ...
    public function search_loan_data($conditions) {
        $result_array = array();
        $loan_data_array = array();

        $select = 'LD.lead_id, LD.loan_no, MS.status_name as lead_status, LC.pancard, ';
        $select .= ' TRIM(CONCAT_WS (" ",LC.first_name, LC.middle_name, LC.sur_name)) as full_name, LC.email, ';
        $select .= ' LC.alternate_email, LC.mobile, LC.alternate_mobile, ';
        $select .= ' Lead_applied_state.m_state_name as customer_lead_applied_state, Lead_applied_city.m_city_name as customer_lead_applied_city, ';
        ...
        if (!empty($tempDetails->num_rows())) {
            ...
            foreach ($tempDetails->result_array() as $columns) {
                ...
                $loan_data_array[] = $data;
            }
            $result_array['data'] = $loan_data_array;
        }
        return $result_array;
    }
}
