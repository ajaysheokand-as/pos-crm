...
class InstantJourneyController extends REST_Controller {
    ...
    public function instantAppVersionCheck_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if (in_array($app_name, array("Salarywalle"))) {

                    if ($version == $current_version || $version = '1.0.0') { // || $version == '3.0.0'
    ...
    public function customerFeedback_post() {
        ...
        try {
            ...
            $feedback_attachment = base64_decode($post['attachment']);
            $feedback_ext = $post['ext'];

            if (empty($feedback_id) || empty($feedback_remark) || empty($feedback_attachment)) {
                throw new ErrorException("Feedback ID, Remark, and Attachment cannot be empty");
            }

            require_once(COMPONENT_PATH . 'CommonComponent.php');
            $this->commonComponent = new CommonComponent();
            $request_array = ['flag' => 1, 'file' => $feedback_attachment, 'ext' => strtotime($feedback_ext)];
