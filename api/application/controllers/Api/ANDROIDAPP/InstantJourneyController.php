...
class InstantJourneyController extends REST_Controller {
    ...
    public function instantAppVersionCheck_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && $header_validation) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($app_name = "Speedoloan") {

                    // if ($version == $current_version || $version = '2.0.0' || $version == '3.0.0') {
    ...
    public function appCustomerRegisteration_post() {
        ...
        $appfyler_uid = !empty($post['appflyer_uid']) ? $post['appflyer_uid'] : NULL;
        $appfyler_advertiser_id = !empty($post['appflyer_advertiser_id']) ? $post['appflyer_advertiser_id'] : NULL;
        ...
        try {
            ...
            switch ($page_name) {
                ...
                case "login":
                    ...
                    if ($insert_profile_flag == 1) {
                        ...
                        if ($cp_data_source_id != $this->data_source_id) {
                            ...
                            $update_customer_profile['cp_adjust_gps_adid'] = $appfyler_advertiser_id;
                        } else if (empty($check_existing_profile['customer_profile_details']['cp_adjust_adid']) || $check_existing_profile['customer_profile_details']['cp_adjust_adid'] == '') {
                            $update_customer_profile['cp_adjust_adid'] = $appfyler_uid;
                        }
                    ...
                    $response_array['appflyer_profile_id'] = $cust_profile_id;
                    $response_array['next_step'] = "otp_verify";
                    $response_array['step_percentage'] = 0;

                    $otp = rand(1111, 9999);

                    if (in_array($mobile, $this->whitelisted_numbers[0])) {
                        $otp = 7488;
                    }
                ...
                case "personal_details":
                    ...
                    if ($get_customer_profile['cp_user_type'] == "REPEAT" || $get_customer_profile['cp_user_type'] == "UNPAID-REPEAT") {
                        unset($update_customer_profile['cp_dob']);
                    }
                ...
                case "register_now":
                    ...
                    $get_otp_trans_logs = $this->Tasks->select_data_by_filter(['lot_profile_id' => intval($cust_profile_id)], 'lot_id', 'leads_otp_trans', 'lot_otp_verify_time DESC', 1);
                    ...
                    if ($check_eligibility['status'] != 1) {
                        ...
                        $eligibility_reason = "You have already performed this action today. Please try again tomorrow.";
                    ...
                    if ($monthly_income <= 25000 && $user_type == "NEW") {
                        ...
                        $eligibility_reason = "You are a little below our salary criteria.";
                        $eligibility_failed_content = array('eligibility_content' => 'We regret to inform you that you are not eligible for a loan based on our internal guidelines.', 'eligibility_reason' => $eligibility_reason);
                        throw new RuntimeException("You are not eligible for a loan.");
                    }
    ...
    public function saveleadDetails_post() {
        ...
        try {
            ...
            if (in_array($leadDetails['lead_status_id'], array(1, 4)) && $leadDetails['user_type'] == 'NEW') {
            ...
            switch ($page_name) {
                ...
                case "loan_quotation_decision":
                    ...
                    } elseif ($loan_quote_accepted_id == 1 && $monthly_income <= 25000) {
                        $lead_status_id = 8;
                        $lead_stage = 'S8';
                ...
                case "banking_details":
                    ...
                    if (($get_affiliate_data->num_rows() > 0) && ($leadDetails['user_type'] == "NEW")) {
    ...
    public function customerFeedback_post() {
        ...
        $apiStatusData = NULL;
        $input_data = file_get_contents("php://input");
        $post = $input_data ? $this->security->xss_clean(json_decode($input_data, true)) : $this->security->xss_clean($_POST);
        $headers = $this->input->request_headers();
        $cust_profile_id = !empty($post["profile_id"]) ? $this->encrypt->decode($post["profile_id"]) : NULL;
        ...
        try {
            ...
            $token_auth = $this->_token();
            $token = $token_auth['token_android'];
            if (($token != base64_decode($headers['Auth'])) && !empty($headers['Auth'])) {
                throw new Exception("Unauthorized Request!");
            }

            $feedback_id = $post['feedback_id'];
            $feedback_remark = $post['remark'];
            $feedback_attachment = $post['attachment'];
            $feedback_ext = $post['ext'];

            if (empty($feedback_id) || empty($feedback_remark)) {
                throw new ErrorException("Feedback ID and Remark cannot be empty");
            }


            if (!empty($feedback_attachment)) {
                if (!in_array($feedback_ext, array("pdf", "jpg", "png", "jpeg", "JPG", "JPEG", "PNG"))) {
                    throw new ErrorException("Feedback attachment should be in pdf, jpg, or png format only.");
                }

                require_once(COMPONENT_PATH . 'CommonComponent.php');
                $this->commonComponent = new CommonComponent();
                $request_array = ['flag' => 1, 'new_file_name' => 'customer_feedback', 'file' => $feedback_attachment, 'ext' => strtolower($feedback_ext)];

                if (API_DOC_S3_FLAG) {
                    $upload_file = $this->commonComponent->upload_document(0, $request_array);
                    $image_name = $upload_file['file_name'];
                } else {
                    $image_name = "feedback_lms_" . date("YmdHis") . "_" . rand(1000, 9999) . "." . strtotime($feedback_ext);
                    $image_upload_dir = UPLOAD_PATH . $image_name;
                    $doc_file = file_put_contents($image_upload_dir, base64_decode($feedback_attachment));
                    $upload_file['status'] = !empty($doc_file) ? 1 : 0;
                }
            ...
            $apiData = [
                'cfr_type_id' => $feedback_id,
                'cfr_customer_id' => $cust_profile_id,
                'cfr_source_id' => 2,
                'cfr_remarks' => $feedback_remark,
                'cfr_attachment' => $image_name
            ];

            $this->Tasks->insert("customer_feedback_responses", $apiData);
            $last_id = $this->db->insert_id();

            if (empty($last_id)) {
                throw new ErrorException("Feedback not submitted. Please try again");
            }

            $response_array = ['id' => $last_id, 'message' => "Your feedback has been submitted successfully"];
            $apiStatusId = 1;
    ...
    public function getCustomerDetails_post() {
        ...
        try {
            ...
            if (($lead_journey_events['lje_loan_quote'] == 1) && ($lead_eligibility_flag == 1)) {
                ...
                // if (in_array($mobile, $this->whitelisted_numbers)) {
                if ($user_type == "REPEAT") {
            ...
            if (!empty($cust_profile_id)) {
                ...
                if ($get_customer_details['status'] == 1) {
                    ...
                    if ($otp_verify_flag == 1 && $status == 1) {
                        $get_otp_verify_status = $this->Tasks->select_data_by_filter(['lot_profile_id' => intval($cust_profile_id)], 'lot_otp_verify_flag', 'leads_otp_trans', 'lot_id DESC', 1);
                    ...
                    if (!empty($lead_id)) {
                        ...
                        if ($get_lead_details['status'] == 1) {
                            ...
                            if (!in_array($lead_details['lead_status_id'], array(1, 4, 41, 42))) {
                                $status = 0;
        ...
        try {
            ...
            if ($active_loan_flag == 1) {
                ...
                if (in_array($active_lead_details['lead_status_id'], array(14, 19))) {
                    ...
                    $CommonComponent->get_loan_repayment_details($active_lead_id);
                    ...
                    $date_diff =  strtotime(date("Y-m-d")) - strtotime($cam_details['disbursal_date']);
                    ...
                    if (isset($loan_recommended, $roi, $remaining_days) && strtotime($cam_details['repayment_date']) > strtotime(date("Y-m-d"))) {
                        $total_due = (($loan_recommended * $roi * $remaining_days) / 100) + $loan_recommended - $loan_details['loan_total_received_amount'];
                    }
                ...
                $get_collection_history = $this->Tasks->lastCollectionHistory($pancard);
                if ($get_collection_history['status'] == 1) {
                    $collection_history = $get_collection_history['collection_history'];
                }

                if (isset($repayment_date) && strtotime("-5 days", strtotime($repayment_date)) > strtotime(date("Y-m-d"))) {
                    $next_event_name = "completed";
                }

                $response_array['profile_details']['user_type'] = "REPEAT";
            ...
            // if (in_array($mobile, $whitelisted_numbers)) {
            //     $journey_whitelisted_flag = 1;
            // }

            $cibil_details = $this->Tasks->get_lastest_cibil_file($pancard);
            if (!empty($cibil_details)) {
                $cibil_data = $cibil_details['cibil_data'];
                $response_array['bureau'] = $cibil_data;
            }
            ...
            $response_array['is_banner'] = 0;

            $lastleaddetails = $this->Tasks->getLastLeadDetails($pancard);
            if ($lastleaddetails['status'] == 1) {
                $lastleaddetails = $lastleaddetails['data'];
            }

            $response_array['is_editable'] = 0;
            if (in_array($lastleaddetails['lead_status_id'], [8, 9, 16, 17, 18]) || empty($lastleaddetails['lead_status_id'])) {
                $response_array['is_editable'] = 1;
            }

            // if (in_array($lastleaddetails['lead_status_id'], [8, 9])) {
            //     $next_event_name = "register_now";
            // }
            // $response_array['is_banner'] = count($app_banners) > 0 ? 1 : 0;
    ...
    public function getExcutiveDetails($user_id) {
        ...
        if ($getExcutiveDetails['status'] == 1) {
    ...
    public function getLeadList_post() {
        ...
        try {
            ...
            if ($get_lead_list['status'] == 1) {
                $data = $get_lead_list['leads_data'];

                foreach ($data as $key => $value) {

                    $lead_list[$key]['lead_id'] = $value['lead_id'];
                    $lead_list[$key]['application_no'] = $value['application_no'];
                    $lead_list[$key]['loan_no'] = $value['loan_no'];
                    $lead_list[$key]['monthly_salary_amount'] = $value['monthly_salary_amount'];
                    $lead_list[$key]['pincode'] = $value['pincode'];
                    $lead_list[$key]['lead_entry_date'] = $value['lead_entry_date'];
                    $lead_list[$key]['loan_amount'] = $value['loan_recommended'] > 0 ? $value['loan_recommended'] : $value['loan_amount'];
                    $lead_list[$key]['tenure'] = strval($value['cam_tenure'] > 0 ? $value['cam_tenure'] : $value['tenure']);

                    if ($value['lead_status_id'] < 4) {
                        $lead_list[$key]['status'] = "Pending";
                    } else if (in_array($value['lead_status_id'], [8, 9])) {
                        $lead_list[$key]['status'] = "Rejected";
                    } else if ($value['lead_status_id'] == 4) {
                        $lead_list[$key]['status'] = "Submitted";
                    } else if ($value['lead_status_id'] == 14) {
                        $lead_list[$key]['status'] = "Disbursed";
                    } else if ($value['lead_status_id'] == 19) {
                        $lead_list[$key]['status'] = "Partially-Paid";
                    } else if ($value['lead_status_id'] == 16) {
                        $lead_list[$key]['status'] = "Closed";
                    } else if ($value['lead_status_id'] == 17) {
                        $lead_list[$key]['status'] = "Settled";
                    } else if ($value['lead_status_id'] == 18) {
                        $lead_list[$key]['status'] = "Write-OFF";
                    } else {
                        $lead_list[$key]['status'] = "Processing";
                    }
                }
            }
    ...
    function createRazorPayOrderID($lead_id, $repayment_data) {
        ...
        // $key_id = "rzp_live_3XXwpvgLtdYIh3";
        // $key_secret = "hwsGRXVuJ5BUjT3KdNaSYc4T";
    ...
    function verifyRazorPayCheckPaymentStatus_post() {
        ...
        try {
            ...
            if ($paymentData['status'] == 'captured') {
                ...
                $apiStatusData = $paymentData;
                $apiStatusId = 1;
                $apiStatusMessage = "Payment captured successfully.";
            } else {
                $apiStatusMessage = "Payment not successful, status: " . $paymentData['status'];
            }
        } catch (Exception $e) {
            $apiStatusMessage = $e->getMessage();
        }
    ...
    public function getLeadDetail_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $select = "SELECT LD.lead_id, LD.loan_no, LD.customer_id, LD.application_no, LD.lead_reference_no, LD.monthly_salary_amount,
                        ...
                        LD.cibil, CE.income_type, CE.salary_mode, CE.monthly_income,
                        ...
                        MQ.m_qualification_name as qualification, IF(CAM.tenure > 0, CAM.tenure,  LD.tenure) as tenure, IF(CAM.loan_recommended > 0, CAM.loan_recommended,  LD.loan_amount) as loan_amount
                        FROM leads LD
