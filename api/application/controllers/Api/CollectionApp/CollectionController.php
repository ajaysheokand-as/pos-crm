...
class CollectionController extends REST_Controller {
    ...
    public function collexLoanDetailPayment_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($token_verification['status'])) {
                ...
                $select = "SELECT L.lead_id from loan L WHERE L.loan_no=" . '"' . $loan_no . '"';
                $loan_data = $this->db->query($select)->row_array();
                $lead_id = $loan_data['lead_id'];

                $select_CAM = "SELECT CAM.roi, CAM.panel_roi from credit_analysis_memo CAM WHERE CAM.lead_id=" . '"' . $lead_id . '"';
                $getCAM_data = $this->db->query($select_CAM)->row_array();
                $roi = $getCAM_data['roi'];
                $panel_roi = $getCAM_data['panel_roi'];

                $get_payment_data = $this->Collex->get_repayment_details($lead_id);
                $get_loan_data = $this->Collex->get_basic_loan_details($lead_id);

                $get_payment_data['roi'] = number_format($roi, 2);
                $get_payment_data['penal_roi'] = number_format($panel_roi, 2);

                $get_payments_data = array_merge($get_payment_data, $get_loan_data['data']['loan_details']);

                if (!empty($get_loan_data) || !empty($get_loan_data)) {
                    unset($this->result_array['error']);
                    $return_data['status'] = 1;
                    $return_data['message'] = "Record Found";
                    // $return_data['data'] = $get_loan_data['data']['loan_details'];

                    $return_data['data']['repayment_details'] = $get_payments_data;
                } else {
    ...
    public function collexUploadPayment_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($token_verification['status'])) {
                ...
                $this->form_validation->set_data($post);
                $this->form_validation->set_rules("lead_id", "lead_id", "required|trim");
                $this->form_validation->set_rules("search_by_lead", "search_by_criteria", "required|trim|numeric");
                $this->form_validation->set_rules("user_id", "user_id", "required|trim");
                $this->form_validation->set_rules("repayment_amount", "repayment_amount", "required|trim");
                $this->form_validation->set_rules("repayment_mode", "repayment_mode", "required|trim");
                    ...
                    if (!empty($lead_details)) {
                        ...
                        if ($leads['lead_status_id'] != 16) {
                            ...
                            if (empty($total_received_amount)) {
                                ...
                                $insert_collection['lead_id'] = $lead_id;
                                $insert_collection['company_id'] = $company_id;
                                $insert_collection['product_id'] = $product_id;
                                $insert_collection['customer_id'] = $customer_id;
                                $insert_collection['loan_no'] = $loan_no;
                                $insert_collection['received_amount'] = $total_received_amount;
                                ...
                                $insert_collection['remarks'] = $remarks;
                                $insert_collection['date_of_recived'] = date('Y-m-d');
                                ...
                                if (empty($collection_inserted_id)) {
                                    ...
                                    $return_data['status'] = 1;
                                    $return_data['message'] = "Payment saved successfully.";
                                    ...
                                    $return_data['status'] = 1;
                                    $return_data['message'] = "Payment update successfully.";
                                }
                                ...
                                if (!empty($upload_payment)) {
                                    ...
                                    if (!empty($docs['status'])) {
                                        ...
                                        $return_data['message'] .= "Screenshot uploaded successfully.";
                                    } else {
                                        $return_data['message'] = "Failed to upload Screenshot. try again";
                                    }
                                }
                            } else {
                                $return_data['message'] = "The entered amount cannot be greater than the total due amount. - Rs. " . $total_due_amount;
                                $this->api_status = REST_Controller::HTTP_OK;
                            }

                            $return_data['status'] = 1;
                            $return_data['message'] = $return_data['message'];
                        } else {
                            $return_data['status'] = 0;
                            $return_data['message'] = "Loan already closed";
                        }
                    } else {
                        $return_data['status'] = 0;
                        $return_data['message'] = "Lead Details Not Found";
                    }
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if (!empty($visit_details->num_rows())) {
                        ...
                        if (in_array($post['visit_trip_status_id'], [1]) && in_array($loan_visit_status_id, [2])) { // Start Visit
                            ...
                            $update_visit['col_fe_visit_trip_start_latitude'] = $post['visit_trip_latitude'];
                            $update_visit['col_fe_visit_trip_start_longitude'] = $post['visit_trip_longitude'];
                            ...
                            $update_visit['col_fe_visit_end_latitude'] = $post['visit_trip_latitude'];
                            $update_visit['col_fe_visit_end_longitude'] = $post['visit_trip_longitude'];
                            ...
                            $update_visit['col_fe_visit_trip_stop_latitude'] = $post['visit_trip_latitude'];
                            $update_visit['col_fe_visit_trip_stop_longitude'] = $post['visit_trip_longitude'];
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if (!empty($cfe_last_visit['status'])) {
                        ...
                        if (!empty($cfe_last_visit_details['col_fe_rtoh_return_type'])) {
                            ...
                            $remarks = addslashes($post['remarks']);
                            $return_visit_type = $post['return_visit_type'];
                            $total_visit_distance = $post['total_visit_distance'];
                            $return_visit_latitude = $post['return_visit_latitude'];
                            $return_visit_longitude = $post['return_visit_longitude'];

                            $lcf_fe_upload_selfie_return_from_visit = $post['upload_selfie']; // 14

                            $update_loan_collection_followup['col_fe_rtoh_return_type'] = $return_visit_type;
                            $update_loan_collection_followup['col_fe_rtoh_remarks'] = $remarks;
                            $update_loan_collection_followup['col_fe_rtoh_return_datetime'] = date("Y-m-d H:i:s");
                            $update_loan_collection_followup['col_fe_rtoh_total_distance_covered'] = $total_visit_distance;
                            $update_loan_collection_followup['col_fe_rtoh_end_latitude'] = $return_visit_latitude;
                            $update_loan_collection_followup['col_fe_rtoh_end_longitude'] = $return_visit_longitude;
            ...
            if (!empty($token_verification['status'])) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if (!empty($lead_details->num_rows())) {
                        ...
                        $condition_visit['col_visit_id'] = $visit_id;
                        $total_visit_distance = $post['total_visit_distance'];
                        $total_received_amount = trim($post['repayment_amount']);
                        $next_collection_followup_date = strtotime($post['next_collection_followup_date']);
                        ...
                        if (in_array($post['collection_followup_id'], [1])) { // UpdateNextCollectionFollowup
                            ...
                            $lcf_fe_upload_selfie = $post['upload_selfie']; // 14
                            $lcf_fe_upload_location = $post['upload_location']; // 14

                            $insert_loan_collection_followup['lcf_lead_id'] = $lead_id;
                            $insert_loan_collection_followup['lcf_next_schedule_datetime'] = $next_collection_followup_date;
                            $insert_loan_collection_followup['lcf_user_id'] = $user_id;
                            $insert_loan_collection_followup['lcf_remarks'] = $remarks;
                            $insert_loan_collection_followup['lcf_created_on'] = date("Y-m-d H:i:s");
                            $insert_loan_collection_followup['total_distance_covered'] = $total_visit_distance;
                        ...
                        if (!empty($visit_id)) {
                            ...
                            if (!empty($remarks)) {
                                $update_visit['col_visit_field_remarks'] = $remarks;
    ...
    public function collexMasterPaymentMethods_post() {
        $post = $this->request(file_get_contents("php://input"));
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($token_verification['status'])) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if ($version == $current_version) {
                        $upi['payment_type'] = '1';
                        $upi['payment_method_name'] = 'upi';
                        $upi['payment_method_details'] = array(
                            'payment_label' => 'UPI',
                            'payment_bank_name' => 'ICICI Bank Ltd.',
                            'payment_company_name' => 'Devmuni Leasing Finance Limited',
                            'payment_upi_id' => 'devmunileasinfinancelimited@icici',
                            'payment_account_number' => NULL,
                            'payment_ifsc_code' => NULL,
                            'payment_branch' => NULL,
                            'payment_account_type' => NULL,
                            'payment_img_url' => WEBSITE_URL . 'public/image/bharatloan-pay-now.jpeg'
                        );

                        $icici_bank['payment_type'] = '2';
                        $icici_bank['payment_method_name'] = 'icici_banking';
                        $icici_bank['payment_method_details'] = array(
                            'payment_label' => 'ICICI Bank',
                            'payment_bank_name' => 'ICICI Bank Ltd.',
                            'payment_company_name' => 'Devmuni Leasing and Finance Ltd.',
                            'payment_upi_id' => NULL,
                            'payment_account_number' => '344305001083',
                            'payment_ifsc_code' => 'ICIC0003443',
                            'payment_branch' => 'ICICI Bank, Rohini Sector 7, Delhi-110085',
                            'payment_account_type' => 'Current Account',
                            'payment_img_url' => WEBSITE_URL . 'public/image/bharatloan-icici-icon.png'
                        );

                        $payment_methods[] = $upi;
                        $payment_methods[] = $icici_bank;

                        $this->result_array['status'] = 1;
                        $this->result_array['data'] = $payment_methods;
                        $this->api_status = REST_Controller::HTTP_OK;
                    } else {
                        $this->result_array['message'] = "Please update the new version.";
                        $this->result_array['version'] = round($current_version, 2);
    ...
    public function collexSearchLoanData_post() {
        $post = $this->request(file_get_contents("php://input"));
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($token_verification['status'])) {
                ...
                if (!empty($post['search_type_id']) && $post['search_type_id'] == 1) {
                    $where['LD.loan_no'] = $post['search_type_value'];
                }

                if (!empty($post['search_type_id']) && $post['search_type_id'] == 2) {
                    $where['LD.pancard'] = $post['search_type_value'];
                }

                if (!empty($post['search_type_id']) && $post['search_type_id'] == 3) {
                    $where['LD.lead_id'] = $post['search_type_value'];
                }
                ...
                $config["per_page"] = 10;
                $page = !empty($post['per_page']) ? intval($post['per_page']) : 0; // 10
                //                        $return_data = $this->Collex->search_total_collection_lists($where, $config["per_page"], $page);
                $return_data = $this->Collex->search_loan_data($where);
                if (!empty($return_data['status'])) {
                    $this->result_array['status'] = 1;
                    $this->result_array['data'] = $return_data['data'];
                    $this->api_status = REST_Controller::HTTP_OK;
    ...
    public function collexMasterSearchType_post() {

        $master_search_type = array(1 => "Loan No", 2 => "Pan", 3 => "Lead Id");
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($token_verification['status'])) {
                ...
                $return_master = null;
                foreach ($master_search_type as $key => $value) {
                    $return_master[] = array("search_type_id" => $key, "search_type_label" => $value);
                }

                $this->result_array['status'] = 1;
                $this->result_array['data'] = $return_master;
                $this->api_status = REST_Controller::HTTP_OK;
            } else {
                $this->result_array['status'] = 440;
                $this->result_array['message'] = "Session Expired.";
