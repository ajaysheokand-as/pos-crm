...
//        }
    ...
    //     $data = $this->getCibilFile($cibil_id);
    //     $file = $data['cibil_file'];
    //   //  $file = str_replace('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">', "", $file);

    //     $file = file_put_contents(APPPATH . "/views/cibil_score_pdf.php", $file);

    //     $pdffile = $this->load->view("cibil_score_pdf",true);

    //    $mpdf = new \Mpdf\Mpdf();
    //    $mpdf->WriteText($pdffile);
    ...
    $datapdf = file_get_contents(APPPATH . "/views/cibil_score_pdf.php", $filename);
    $mpdf = new \Mpdf\Mpdf();
    $html = $this->load->view('cibil_score_pdf', $datapdf, true);
    $mpdf->WriteHTML($html);
    $mpdf->defaultfooterline = 1;
    $mpdf->Output();
