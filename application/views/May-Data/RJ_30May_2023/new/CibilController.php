...
//        }
    ...
    public function viewCustomerCibilPDF($cibil_id) {
        ...
        // $pdffile = $this->load->view("cibil_score_pdf");

        $data = $this->getCibilFile($cibil_id);
        $filename = $data['cibil_file'];
        
        $data = file_put_contents(APPPATH . "/views/cibil_score_pdf.php", $filename);
      
        $mpdf = new \Mpdf\Mpdf();
        $html = $this->load->view('cibil_score_pdf', $data, true);
        
        $getdata=strip_tags($html);
        $mpdf->WriteHTML($getdata);
