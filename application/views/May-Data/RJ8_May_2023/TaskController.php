...
//            $where_in['LD.lead_rejected_reason_id'] = [7, 31];
    ...
    //////////////////////////  Function Create By Rohit Jain 5 April 2023 /////////////////////

    public function getRejectDetails($leadId) {
        ...
        $conditions['LD.lead_id'] = $lead_id;
        $leadData = $this->Tasks->getRejectDetails($conditions);
        $data['leadDetails'] = $leadData->row();
    ...
    public function getEmpOccupation() {
        $stateArr = $this->Tasks->getEmpOccupation();
        $json['EmpOccupation'] = $stateArr->result();
        echo json_encode($json);
    }
        ...
        if (!empty($lead_id)) {
            if (isset($_FILES['file_name']['name'])) {
                ...
                if ($upload_return['status'] == 1) {
                    $docs = $upload_return['file_name'];

//                $extension = strtolower(pathinfo($docs, PATHINFO_EXTENSION));
    ...
    public function rejectedLeadMoveToProcess() {
        ...
        if (empty($_SESSION['isUserSession']['user_id'])) {
            ...
            if (!empty($lead_id)) {
                ...
                $leadDetails = $this->Tasks->select(['lead_id' => $lead_id], 'lead_id, lead_status_id, lead_rejected_reason_id', 'leads');

                if ($leadDetails->num_rows() > 0) {

                    $leadDetails = $leadDetails->row();

                    if ($leadDetails->lead_status_id == 9 && in_array($leadDetails->lead_rejected_reason_id, array(7, 31))) {
...
//                        $sql2 = $this->Tasks->select($conditions2, $select2, 'users');
    ...
    public function sanctionleads() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $sql .= " ,CAM.cam_processing_fee_gst_type_id";
            $sql .= " FROM leads LD";
...
//            if (ENVIRONMENT == 'production') {
            ...
            if ($leadDetails['user_type'] != "REPEAT" || $aadhaarDocsDetails['status'] == 1) {
                ...
                if ($aadhaar_ocr_return['status'] == 1) {
                    ...
                    $this->Tasks->sent_ekyc_request_sms($lead_id);
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($leadDetails->num_rows() > 0) {
                    ...
                    $occupation_id= $this->input->post('customer_spouse_occupation_id');
                    $qualification_id= $this->input->post('customer_qualification_id');
    ...
    public function insertResidence() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $lead_update = array();
                $lead_branch_id = 0;
                $conditions = ['C.customer_lead_id' => $this->input->post('lead_id')];
                ...
                $conditionsLead = ['lead_id' => $this->input->post('lead_id')];
    ...
    public function insertEmployment() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $employment_data['emp_website'] = $this->input->post('website');
