    ...
    public function collexUploadPayment_post() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if (!empty($token_verification['status'])) {
                ...
                $refrence_no = $post['refrence_no'];
                $payment_mode_id = $post['payment_mode_id'];
                $payment_mode_name = $post['payment_mode'];
                $ip = $post['ip'];
                $cl_remarks = $post['remarks'];
                $collected_by = $post['collection_executive_user_id'];

                $image_upload_dir = '';
                $docs_file = $_FILES["file"]["name"];
                $image_name = $lead_id  . "_lms_" . date("YmdHis") . "_" . rand(1000, 9999);
                $targetFilePath = $image_name . $docs_file;
                $fileType = pathinfo($targetFilePath,PATHINFO_EXTENSION);
                $allowTypes = array('jpg','png','jpeg','jpg');
                $imgUrl = $image_name.'_'.$docs_file;

                if(in_array($fileType, $allowTypes)){
                    if( file_put_contents($targetFilePath, base64_decode($docs_file))){

                        $image_upload_dir = UPLOAD_PATH . $docs_file;
                    }else{
                        $image_upload_dir = "Sorry, there was an error uploading your file.";
                    }
                }
                else{
                    $image_upload_dir = 'Sorry, only JPG, JPEG, PNG, GIF, & PDF files are allowed to upload.';
                }

                $insertCollectionData = array(
                    "lead_id" => $lead_id,
                    "company_id" => 1,
                    "product_id" => 1,
                ...
                $result = $this->db->insert('collection', $insertCollectionData);

                $return_data['status'] = 1;
                $return_data['message'] = "Inserted Successfully";
