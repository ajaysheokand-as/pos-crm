...
//                echo $this->db->last_query();
    ...
    public function getEmploymentDetails($lead_id) {
        ...
        if (!empty($lead_id)) {
            $conditions = ['LD.lead_id' => $lead_id];
            $select = 'CE.city_id,CE.state_id,CE.customer_id, CE.employer_name, CE.emp_state, CE.emp_city, CE.emp_district, CE.emp_pincode, CE.emp_house, CE.emp_street, CE.emp_landmark, CE.emp_residence_since, CE.presentServiceTenure, CE.emp_designation, CE.emp_department, CE.emp_employer_type, CE.emp_website, CE.monthly_income, CE.salary_mode, CE.income_type, CE.industry, CE.sector, CE.salary_mode, CE.emp_status, CE.created_on, ST.m_state_name, CT.m_city_name, department.department_name';
            ...
            foreach ($sql->result() as $row) {
                ...
                $data .= '
                        <tr>
                            ...
                            <td class="whitespace">' . htmlspecialchars($row->loan_no) . '</td>
                            <td class="whitespace"><div class="tooltip"><i class="fa fa-comment"></i><span class="tooltiptext">
                                    <b>Remarks Collection : </b><i>' . htmlspecialchars($row->remarks) . '</i><br/>
                                    <b>Remarks Closure : </b><i>' . htmlspecialchars($row->closure_remarks) . '</i>
                                    </span></div></td>
                            <td class="whitespace">' . htmlspecialchars($row->payment_mode) . '</td>
                            <td class="whitespace">' . htmlspecialchars($row->received_amount) . '</td>
                            <td class="whitespace">' . htmlspecialchars($row->discount) . '</td>
                            <td class="whitespace">' . htmlspecialchars($row->refund) . '</td>
                            <td class="whitespace">' . htmlspecialchars($row->refrence_no) . '</td>
                            <td class="whitespace">' . $date_of_recived . '</td>
                            <td class="whitespace">' . htmlspecialchars($repaymentstatus->status_name) . '</td>
                            <td class="whitespace">' . htmlspecialchars($payment_verification ). '</td>
                            <td class="whitespace">' . htmlspecialchars($row->collection_executive_name) . '</td>
                            ...
                            <td class="whitespace">' . (($row->closure_user_name == null) ? '-' : htmlspecialchars($row->closure_user_name)) . '</td>
...
//        $wherecond = '(status = "SANCTION" OR status= "DISBURSE-PENDING" OR status= "DISBURSED" OR status= "SETTLED" OR status= "WRITEOFF" OR status= "PART-PAYMENT")';
    ...
    public function getCAMDetails($lead_id) {
        ...
        $select = 'LD.lead_id, LD.user_type, LD.lead_data_source_id, LD.customer_id, LD.application_no, C.first_name, C.middle_name, C.sur_name, C.email, C.alternate_email, LD.pancard, LD.cibil, ';
        ...
        $select .= ' CAM.eligible_foir_percentage, CAM.admin_fee as processing_fee_percent, ';
    ...
    public function sendSanctionMail($lead_id) {
        ...
        $return_array = lw_send_email($email, $subject, $message, $bcc_email, "", "", "", UPLOAD_PATH, $cam_sanction_letter_file_name);

        if (!empty($alternate_email)) {
            $return_array = lw_send_email($alternate_email, $subject, $message, $bcc_email, "", "", "", UPLOAD_PATH, $cam_sanction_letter_file_name);
        }

        return $return_array;
    }

    public function sendSanctionSMS($lead_id){
        $sms_type_id=12;
        $req = array();
