...
//            echo $this->db->last_query(); exit;
    ...
    public function get_LoanDetail_payment_mode($loan_no){
        ...
        $select = 'LD.lead_id, LD.loan_no, LD.customer_id, LD.ip';
        ...
        if (!empty($LoanDetails->num_rows())) {
            ...
            $user_select = 'SELECT user_id FROM users WHERE user_status_id= 1 AND is_Active=1 AND user_active=1';
            $user_mode = $this->db->query($user_select)->num_rows();
        
          

            $data_array['status'] = 1;
            $data_array['message'] = "Record Found";
            $data_array['data']['loan_details'] = $row_data[0];
            $data_array['data']['user_details'] = $user_mode; 
       
            $data_array['data']['payment_details'] = $paymet_mode;         
            //$data_array['data']['collection_detais'] = $collection_mode; 
            // if(($data_array['data']['collection_detais']['loan_no']!=$data_array['data']['loan_details']['loan_no'])){  
            //     $this->db->insert('collection',$data_array['data']['loan_details'], $data_array['data']['user_details']);
            // }
            }else{       
