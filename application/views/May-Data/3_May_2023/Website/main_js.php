...
<?php } ?>
    ...
    function sanctionFeedback(lead_id, user_id, customer_id)
    {
        $('#btn_own_reason').html('<button class="btn btn-primary lead-sanction-button" style="background : #0a5e90 !important;" onclick="sanctionleads(' + lead_id + ', ' + user_id + ')">Sanction</button>');
    ...
    function sanctionleads(lead_id, user_id, customer_id)
    {
        ...
        if (lead_id == "") {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {lead_id: lead_id, user_id: user_id, remarks: sanction_remarks, csrf_token},
                dataType: 'json',
                beforeSend: function () {
    ...
    {
        $('#btn_own_reason').html('<button class="btn btn-primary" onclick="processLeadSendBack(<?= $leadDetails->lead_id ?>)">Send Back</button>');
        $('#divExpendReason3').toggle();
    }
    ...
    $('#divExpendReason3').hide();
    function disburseSendBack(lead_id, user_id, customer_id)
    {
        $('#btn_own_reason').html('<button class="btn btn-primary" id="btnRejectApplication" onclick="disbursalSendBack(<?= $leadDetails->lead_id ?>)">Disbursal Send Back</button>');
        $('#divExpendReason3').toggle();
    }
...
//                html += '<tr><th class="thbg">Advance IRR Amount (Rs.)</th><td>' + ((res.cam_interest_amount) ? res.cam_interest_amount : "-") + '</td><th class="thbg"></th><td></td></tr>';
                ...
                html += '<tr><th class="thbg">Disbursal Email Delivery status</th><td>' + ((res.loanAgreementRequest == 1) ? "SENT" : 'PENDING') + '</td><th class="thbg">Disbursal Email Response status</th><td>' + ((res.loanAgreementResponse == 1) ? "ACCEPTED" : '-') + '</td></tr>';
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.errSession) {
                    ...
                    window.location.href = "<?= base_url("inProcess/S2") ?>";
                } else {
                    catchError(response.err);
                }
            },
...
<?php } ?>
    ...
    {
        var customer_id = "<?= $leadDetails->customer_id ?>";
        var own_remark = $("#own_remark").val();
...
//                        window.location.reload();
        ...
        $("#saveReference").on('click', function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    getReferenceDetails(<?= $leadDetails->lead_id ?>);
                    if (response.msg) {
        ...
        $("#saveBeneficiary").on('click', function (e) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.msg) {
                        getCustomerBanking(<?= $leadDetails->lead_id ?>);
                    ...
                    (function timer() {
                        ...
                        setTimeout(function () {
                            if (i == 0) {
                                ...
                                $('#div_bank_statement_analysis').html('<button class="btn btn-info" id="api_download_bank_statement" onclick="api_download_bank_statement( <?= $leadDetails->lead_id ?> )">Download Cart API Data</button>').prop('disabled', false);
                            } else {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.errSession) {
                    ...
                    if(response.customer_bre_run_flag==1){
                        $("#LeadBREResultInner p").html("BRE Requirement  : Please ensure all the data points and apis has been called before run the BRE. If not then you will get the rejection and repetition of work leads to lower productivity."+
                        "<br/><button onclick='call_bre_edit_application()' class='btn btn-success lead-sanction-button'>Edit Application</button>");
                    }
