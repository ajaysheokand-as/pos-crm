...
class Report_Model extends CI_Model
{
    ...
    public function CollectionMonthwiseReport($fromDate)
    {    
        if (!empty($fromDate)) {
        $fromDate = date('Y-d-m', strtotime($fromDate));   
        $sql = "SELECT LD.lead_id, LD.lead_status_id, CAM.disbursal_date, L.loan_principle_outstanding_amount, CO.received_amount, CO.date_of_recived  FROM leads LD INNER JOIN credit_analysis_memo CAM ON(LD.lead_id=CAM.lead_id) INNER JOIN loan L ON(LD.lead_id=L.lead_id) INNER JOIN collection CO ON(CO.lead_id = LD.lead_id) ";
        $sql .= "WHERE LD.lead_id=CAM.lead_id AND LD.lead_id=L.lead_id AND LD.lead_status_id IN(14,16,17,18,19) AND CO.collection_active=1 AND CO.collection_deleted=0 AND CO.payment_verification=1 AND CO.date_of_recived  <= '$fromDate'"; // 
          
        $coll_result = $this->db->query($sql)->result_array();

        $sql1 = "SELECT LD.lead_id, LD.lead_status_id, CAM.disbursal_date, L.loan_principle_outstanding_amount FROM leads LD INNER JOIN credit_analysis_memo CAM ON(LD.lead_id=CAM.lead_id) INNER JOIN loan L ON(LD.lead_id=L.lead_id)";
        $sql1 .= "WHERE LD.lead_id=CAM.lead_id AND LD.lead_id=L.lead_id AND LD.lead_status_id IN(14,16,17,18,19) AND CAM.repayment_date <= '$fromDate'"; // 
          
        $loan_result = $this->db->query($sql1)->result_array();

        $sql2 = "SELECT LD.lead_id, LD.lead_status_id, CAM.disbursal_date,  CAM.repayment_date, L.loan_principle_outstanding_amount, CO.received_amount, CO.date_of_recived  FROM leads LD INNER JOIN credit_analysis_memo CAM ON(LD.lead_id=CAM.lead_id) INNER JOIN loan L ON(LD.lead_id=L.lead_id) INNER JOIN collection CO ON(CO.lead_id = LD.lead_id) ";
        $sql2 .= "WHERE LD.lead_id=CAM.lead_id AND LD.lead_id=L.lead_id AND CO.collection_active=1 AND CO.collection_deleted=0 AND CO.payment_verification=1 AND CO.date_of_recived  > CAM.repayment_date"; // 
       // echo $sql2; die;
        $advance_result = $this->db->query($sql2)->result_array();
        
                if(!empty($coll_result)){

                    $loan_array = array();
                    foreach($loan_result as $val){
                        $total_outstanding_amount += $val['loan_principle_outstanding_amount'];
                        $disbursal_month1 = date('M-y', strtotime($val['disbursal_date']));
                        $loan_array[$disbursal_month1]['outstanding_amount'] += $val['loan_principle_outstanding_amount'];
                        $loan_array[$disbursal_month1]['no_of_case'] += 1;
                    }
                    foreach($coll_result as $value){
                        $total_collection_amount += $value['received_amount'];
                        $disbursal_month = date('M-y', strtotime($value['disbursal_date']));
                        $loan_array[$disbursal_month]['total_receive_amount'] += $value['received_amount'];

                    }

                    foreach($advance_result as $advance){
                        $repayment_date = date('d-M-y', strtotime($advance['repayment_date']));
                        $total_advance_amount['total_advance_payment'] += $advance['received_amount'];
                    }
                }

                $data = '<table class="bordered" style="width:40%"><thead><tr><th colspan="7" class="footer-tabels-text">MONTH WISE SEGREGATION OF PAYMENT RECEIVED <br> ON ' . date('d-M-Y', strtotime($fromDate)) . '</th></tr>';
                $data .= '<th width="24%" class="no-of-case">MONTH</th>';
                $data .= '<th width="24%" class="no-of-case">No. OF CASE</th>';
                ...
                foreach($loan_array as $key => $val){
                    $data .= '<tr><td>'.$key.'</td> <td>'.$val['no_of_case'].'</td> <td>'.$val['outstanding_amount'].'</td> <td>'.$val['total_receive_amount'].'</td></tr>';
               
                }                
                $data .= '<tr><td>TOTAL</td> <td>---</td>  <td class="text-right">'.$total_outstanding_amount.'</td> <td class="text-right">'.$total_collection_amount.'</td></tr>';
                $data .= '<tr><th colspan="7" class="footer-tabels-text"></th>';
                $data .= '<tr><td colspan="2" style="text-align:left;font-weight:bold;">Advance Payment Recd. for Due <br> Date '.$repayment_date.' </td> <td colspan="2">'.$total_advance_amount['total_advance_payment'].'</td></tr>';
                $total_amount_payment = $total_collection_amount + $total_advance_amount['total_advance_payment'];  
                $data .= '<tr><td colspan="2" style="text-align:left;font-weight:bold;">Total Payment Recd. (A + B)</td> <td colspan="2">'.$total_amount_payment.'</td></tr>';
                $data .= '</tbody></table>';

            //  traceObject($total_advance_amount); die;
