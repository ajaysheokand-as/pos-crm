    ...
    public function get_list_collection_followup($lead_id) {
        ...
        if (!empty($followup_array->num_rows())) {
            foreach ($followup_array->result() as $colum) {
                ...
                $data .= '<tbody>
                        <tr>
                            <td class="whitespace">' . (($colum->lcf_id) ? strtoupper($colum->lcf_id) : '-') . '</td>
                            <td class="whitespace">' . (($colum->followup_username) ? strtoupper($colum->followup_username) : '-') . '</td>
                            <td class="whitespace">' . (($colum->lcf_created_on) ? date("d-m-Y H:i:s", strtotime($colum->lcf_created_on)) : '-') . '</td>                           
                            <td class="whitespace">' . (($colum->m_followup_type_icons) ? '<i class="' . $colum->m_followup_type_icons . '" aria-hidden="true" title="' . $colum->m_followup_type_heading . '"></i>' : "-") . '</td>
                            ';
                ...
                if (in_array($colum->m_followup_type_id, [2])) {
                    ...
                    $status_heading = '<td class="whitespace">' . (($colum->m_followup_status_heading) ? strtoupper($colum->m_followup_status_heading) : "-") . '</td>';
                } else {
                ...
                $data .= '
                            <td class="whitespace">
                                <div class="tooltip"><i class="fa fa-comment"></i><span class="tooltiptext">
                                    <i>' . strtoupper($colum->lcf_remarks) . '</i>
                                    </span>
                                </div>
                            </td>
...
//                    $data .= '<td class="whitespace"><button name="' . $image_upload_selfie . '" onclick="viewDocs(this.name)"><i class="fa fa-file-image-o"></i></button></td>';
                    $data .= '<td class="whitespace"><a class="btn btn-control" target="_blank" href="' . base_url('view-document-file/' . $colum->lcf_id . '/5') . '" title="' . $colum->lcf_id . '"><i class="fa fa-file-image-o"></i></a></td>';
                } else {
                    $data .= $empty_td;
                }
...
//                    $data .= '<td class="whitespace"><button name="' . $image_upload_location . '" onclick="viewDocs(this.name)"><i class="fa fa-file-image-o"></i></button></td>';
                    $data .= '<td class="whitespace"><a class="btn btn-control" target="_blank" href="' . base_url('view-document-file/' . $colum->lcf_id . '/6') . '" title="' . $colum->lcf_id . '"><i class="fa fa-file-image-o"></i></a></td>';
                } else {
                    $data .= $empty_td;
                }
                $data .= '<td class="whitespace">' . (($colum->total_distance_covered) ? $colum->total_distance_covered . "&nbsp;Km" : "-") . '</td>
                        </tr>';
            }
        } else {
        ...
        if (!empty($visit_array->num_rows())) {
            foreach ($visit_array->result() as $colum) {
                ...
                $input = [
                    ...
                    'col_visit_type' => $colum->col_visit_address_type,
                    'col_visit_address_type' => $colum->col_visit_address_type
                ];
                ...
                $data .= '<tbody>
                        <tr>
                            <td class="whitespace">' . (($colum->col_visit_id) ? strtoupper($colum->col_visit_id) : '-') . '</td>
                            <td class="whitespace">' . (($col_visit_address_type) ? strtoupper($col_visit_address_type) : '-') . '</td>
                            <td class="whitespace">' . (($colum->co_excutive_name) ? strtoupper($colum->co_excutive_name) : '-') . '</td>
                            <td class="whitespace">' . (($colum->col_visit_requested_datetime) ? date("d-m-Y H:i:s", strtotime($colum->col_visit_requested_datetime)) : '-') . '</td>
                            <td class="whitespace">
                                <div class="tooltip"><i class="fa fa-comment"></i><span class="tooltiptext">
                            ...
                            <td class="whitespace">' . (($colum->scm_user_name) ? strtoupper($colum->scm_user_name) : '-') . '</th>';

                if (!empty($colum->col_visit_scm_remarks)) {
                    $data .= '<td class="whitespace">
                                        <div class="tooltip"><i class="fa fa-comment"></i><span class="tooltiptext">
                                            <i>' . strtoupper($colum->col_visit_scm_remarks) . '</i>
                                            </span>
                                        </div>
                                    </td>';
                } else {
                ...
                $data .= '<td class="whitespace">' . (($col_visit_field_status) ? strtoupper($col_visit_field_status) : '-') . '</td>
                            ...
                            <td class="whitespace">' . (($colum->rm_user_name) ? strtoupper($colum->rm_user_name) : '-') . '</td>
                ...
                $data .= '<td class="whitespace">' . (($colum->col_fe_visit_total_distance_covered) ? $colum->col_fe_visit_total_distance_covered : '-') . '</td>';
                ...
                $data .= '
                            ...
                            <td class="whitespace">' . (($colum->col_fe_rtoh_total_distance_covered) ? $colum->col_fe_rtoh_total_distance_covered . "&nbsp;Km" : '-') . '</td>
                            ';
                if (!empty($colum->col_fe_rtoh_remarks)) {
                    $data .= '<td class="whitespace">
                                    <div class="tooltip"><i class="fa fa-comment"></i><span class="tooltiptext">
                                        <i>' . (($colum->col_fe_rtoh_remarks) ? strtoupper($colum->col_fe_rtoh_remarks) : "-") . '</i>
                                        </span>
                                    </div>
                                </td>';
                } else {
...
//                    $data .= '<td class="whitespace"><button name="' . $image_upload_selfie . '" onclick="viewDocs(this.name)"><i class="fa fa-file-image-o"></i></button></td>';
                    $data .= '<td class="whitespace"><a class="btn btn-control" target="_blank" href="' . base_url('view-document-file/' . $colum->col_visit_id . '/7') . '" title="' . $colum->col_visit_id . '"><i class="fa fa-file-image-o"></i></a></button></td>';
                } else {
                    $data .= $empty_td;
                }
                $data .= '<td class="whitespace">' . $col_fe_visit_approval_status . '</td>
                            <td class="whitespace">' . ($colum->visit_approved_user_name ? $colum->visit_approved_user_name : "-") . '</td>
...
//            echo "email_data : <pre>"; print_r($email_data); exit;
    ...
    public function get_scm_rm_roles(){
        ...
        $sql.='  where UR.user_role_supervisor_role_id='.$_SESSION['isUserSession']['user_role_id'];
