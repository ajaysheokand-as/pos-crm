    ...
    public function get_list_collection_followup($lead_id) {
        ...
        if (!empty($followup_array->num_rows())) {
            foreach ($followup_array->result() as $colum) {
                ...
                $data .= '<tbody>
                        <tr>
                            <td class="whitespace">' . (($colum->lcf_id) ? strtoupper(htmlspecialchars($colum->lcf_id)) : '-') . '</td>
                            <td class="whitespace">' . (($colum->followup_username) ? strtoupper(htmlspecialchars($colum->followup_username)) : '-') . '</td>
                            <td class="whitespace">' . (($colum->lcf_created_on) ? date("d-m-Y H:i:s", strtotime($colum->lcf_created_on)) : '-') . '</td>                           
                            <td class="whitespace">' . (($colum->m_followup_type_icons) ? '<i class="' . htmlspecialchars($colum->m_followup_type_icons) . '" aria-hidden="true" title="' . htmlspecialchars($colum->m_followup_type_heading) . '"></i>' : "-") . '</td>
                            ';
                ...
                if (in_array($colum->m_followup_type_id, [2])) {
                    ...
                    $status_heading = '<td class="whitespace">' . (($colum->m_followup_status_heading) ? strtoupper(htmlspecialchars($colum->m_followup_status_heading)) : "-") . '</td>';
                } else {
...
//                    $data .= '<td class="whitespace"><button name="' . $image_upload_location . '" onclick="viewDocs(this.name)"><i class="fa fa-file-image-o"></i></button></td>';
                    $data .= '<td class="whitespace"><a class="btn btn-control" target="_blank" href="' . base_url('view-document-file/' . htmlspecialchars($colum->lcf_id) . '/6') . '" title="' . htmlspecialchars($colum->lcf_id) . '"><i class="fa fa-file-image-o"></i></a></td>';
                } else {
                    $data .= $empty_td;
                }
                $data .= '<td class="whitespace">' . (($colum->total_distance_covered) ? htmlspecialchars($colum->total_distance_covered) . "&nbsp;Km" : "-") . '</td>
                        </tr>';
            }
        } else {
