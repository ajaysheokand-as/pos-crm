            ...
            if (!empty($lead_details->num_rows())) {
                ...
                if ($collection_followup_type_id == 1) { // call
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $followup_type_id = $this->input->post('collection_followup_type_id');
                        $followup_status_id = $this->input->post('collection_followup_status_id');
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $sms_data['sms_primary_id'] = $this->input->post("collection_followup_sms_primary_id");
                        $sms_data['sms_template_content'] = $this->input->post("collection_followup_sms_content");
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $email_data['email_template_id'] = $this->input->post("c_followup_email_template_id");
                        $email_data['email_subject'] = $this->input->post("email_subject");
                        $email_data['email_cc_user'] = $this->input->post("email_cc_user");
                        $email_data['email_body'] = $this->input->post("email_body");
...
//            $this->form_validation->set_rules('customer_id', 'Customer Id', 'required|trim');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $closure_remarks = $this->input->post('ops_remarks');
                $payment_verification = $this->input->post('payment_verification');
                $paymentSlips = "";
    ...
    public function addToBlackList() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $reason_remark = $this->input->post('remark');
